	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false)
	{
		...
		if ($items)
		{
			...
			foreach ($getHeadData['scripts'] as $script => $option)
			{
				...
				{
					if (!empty($checkLoadJS['google-maps']) && empty($getHeadData['scripts']['https://maps.google.com/maps/api/js?v=3.21&sensor=false&libraries=places']))
					{
						if ($uri->getScheme() == 'https')
						{
							$scripts['https://maps.google.com/maps/api/js?v=3.21&sensor=false&libraries=places'] = $option;
						}
						else
						{
							$scripts['http://maps.google.com/maps/api/js?v=3.21&sensor=false&libraries=places'] = $option;
						}
			...
			if ($checkLoadScript)
			{
				...
				if (!empty($checkLoadJS['google-maps']))
				{
					if ($uri->getScheme() == 'https')
					{
						$document->addScript('https://maps.google.com/maps/api/js?v=3.21&sensor=false&libraries=places');
					}
					else
					{
						$document->addScript('http://maps.google.com/maps/api/js?v=3.21&sensor=false&libraries=places');
					}
