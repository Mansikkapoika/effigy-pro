!function(){function isArraylike(obj){var length=obj.length,type=jQuery.type(obj);return"function"===type||jQuery.isWindow(obj)?!1:1===obj.nodeType&&length?!0:"array"===type||0===length||"number"==typeof length&&length>0&&length-1 in obj}function winnow(elements,qualifier,not){if(jQuery.isFunction(qualifier))return jQuery.grep(elements,function(elem,i){return!!qualifier.call(elem,i,elem)!==not});if(qualifier.nodeType)return jQuery.grep(elements,function(elem){return elem===qualifier!==not});if("string"==typeof qualifier){if(risSimple.test(qualifier))return jQuery.filter(qualifier,elements,not);
...
}function completed(){(document.addEventListener||"load"===event.type||"complete"===document.readyState)&&(detach(),jQuery.ready())}function dataAttr(elem,key,data){if(void 0===data&&1===elem.nodeType){var name="data-"+key.replace(rmultiDash,"-$1").toLowerCase();if(data=elem.getAttribute(name),"string"==typeof data){try{data="true"===data?!0:"false"===data?!1:"null"===data?null:+data+""===data?+data:rbrace.test(data)?jQuery.parseJSON(data):data}catch(e){}jQuery.data(elem,key,data)}else data=void 0}return data
}function isEmptyDataObject(obj){var name;for(name in obj)if(("data"!==name||!jQuery.isEmptyObject(obj[name]))&&"toJSON"!==name)return!1;return!0}function internalData(elem,name,data,pvt){if(jQuery.acceptData(elem)){var ret,thisCache,internalKey=jQuery.expando,isNode=elem.nodeType,cache=isNode?jQuery.cache:elem,id=isNode?elem[internalKey]:elem[internalKey]&&internalKey;if(id&&cache[id]&&(pvt||cache[id].data)||void 0!==data||"string"!=typeof name)return id||(id=isNode?elem[internalKey]=deletedIds.pop()||jQuery.guid++:internalKey),cache[id]||(cache[id]=isNode?{}:{toJSON:jQuery.noop}),("object"==typeof name||"function"==typeof name)&&(pvt?cache[id]=jQuery.extend(cache[id],name):cache[id].data=jQuery.extend(cache[id].data,name)),thisCache=cache[id],pvt||(thisCache.data||(thisCache.data={}),thisCache=thisCache.data),void 0!==data&&(thisCache[jQuery.camelCase(name)]=data),"string"==typeof name?(ret=thisCache[name],null==ret&&(ret=thisCache[jQuery.camelCase(name)])):ret=thisCache,ret
...
}(),jQuery.version="4.0",jQuery.long_version="4.0.35",jQuery.uid=function(p,s){return(p?p:"")+Math.random().toString().replace(".","")+(s?s:"")},jQuery.globalNamespace=jQuery.uid("Foundry"),window[jQuery.globalNamespace]=jQuery,jQuery.run=function(command){return jQuery.isFunction(command)?command(jQuery):null},jQuery.initialize=function(options){jQuery.initialized||(jQuery.extend(jQuery,options),"development"==jQuery.environment&&(window.F=jQuery),jQuery.initialized=!0,FD40.module.execute())},FD40.jquery(jQuery),jQuery
}(),FD40.plugin("lodash",function($){function baseIndexOf(array,value,fromIndex){for(var index=(fromIndex||0)-1,length=array?array.length:0;++index<length;)if(array[index]===value)return index;return-1}function cacheIndexOf(cache,value){var type=typeof value;if(cache=cache.cache,"boolean"==type||null==value)return cache[value]?0:-1;"number"!=type&&"string"!=type&&(type="object");var key="number"==type?value:keyPrefix+value;return cache=(cache=cache[type])&&cache[key],"object"==type?cache&&baseIndexOf(cache,value)>-1?0:-1:cache?0:-1
...
cloneableClasses[funcClass]=!1,cloneableClasses[argsClass]=cloneableClasses[arrayClass]=cloneableClasses[boolClass]=cloneableClasses[dateClass]=cloneableClasses[numberClass]=cloneableClasses[objectClass]=cloneableClasses[regexpClass]=cloneableClasses[stringClass]=!0;var debounceOptions={leading:!1,maxWait:0,trailing:!1},descriptor={configurable:!1,enumerable:!1,value:null,writable:!1},objectTypes={"boolean":!1,"function":!0,object:!0,number:!1,string:!1,undefined:!1},stringEscapes={"\\":"\\","'":"'","\n":"n","\r":"r","	":"t","\u2028":"u2028","\u2029":"u2029"},root=objectTypes[typeof window]&&window||this,freeExports=objectTypes[typeof exports]&&exports&&!exports.nodeType&&exports,freeModule=objectTypes[typeof module]&&module&&!module.nodeType&&module;
freeModule&&freeModule.exports===freeExports&&freeExports;var freeGlobal=objectTypes[typeof global]&&global;!freeGlobal||freeGlobal.global!==freeGlobal&&freeGlobal.window!==freeGlobal||(root=freeGlobal);var _=$._=runInContext();$.each(_,function(prop,func){Object.hasOwnProperty.call(_,prop)&&!/VERSION/.test(name)&&($[prop]||($[prop]=func))})}),FD40.plugin("bootstrap3",function($){var jQuery=$;if(window["Foundry/Bootstrap"])throw new Error("An instance of Bootstrap has been initialized before this.");window["Foundry/Bootstrap"]={version:"3.0.3",foundry:jQuery},+function($){"use strict";
...
};Button.DEFAULTS={loadingText:"loading..."},Button.prototype.setState=function(state){var d="disabled",$el=this.$element,val=$el.is("input")?"val":"html",data=$el.data();state+="Text",data.resetText||$el.data("resetText",$el[val]()),$el[val](data[state]||this.options[state]),setTimeout(function(){"loadingText"==state?$el.addClass(d).attr(d,d):$el.removeClass(d).removeAttr(d)},0)},Button.prototype.toggle=function(){var $parent=this.$element.closest('[data-bs-toggle="buttons"]'),changed=!0;if($parent.length){var $input=this.$element.find("input");
...
})},$.fn.button.Constructor=Button,$.fn.button.noConflict=function(){return $.fn.button=old,this},$(document).on("click.bs.button.data-api","[data-bs-toggle^=button]",function(e){var $btn=$(e.target);$btn.hasClass("btn")||($btn=$btn.closest(".btn")),$btn.button("toggle"),e.preventDefault()})}(jQuery),+function($){"use strict";var Carousel=function(element,options){this.$element=$(element),this.$indicators=this.$element.find(".carousel-indicators"),this.options=options,this.paused=this.sliding=this.interval=this.$active=this.$items=null,"hover"==this.options.pause&&this.$element.on("mouseenter",$.proxy(this.pause,this)).on("mouseleave",$.proxy(this.cycle,this))
...
data||$this.data("bs.carousel",data=new Carousel(this,options)),"number"==typeof option?data.to(option):action?data[action]():options.interval&&data.pause().cycle()})},$.fn.carousel.Constructor=Carousel,$.fn.carousel.noConflict=function(){return $.fn.carousel=old,this},$(document).on("click.bs.carousel.data-api","[data-bs-slide], [data-bs-slide-to]",function(e){var href,$this=$(this),$target=$($this.attr("data-target")||(href=$this.attr("href"))&&href.replace(/.*(?=#[^\s]+$)/,"")),options=$.extend({},$target.data(),$this.data()),slideIndex=$this.attr("data-bs-slide-to");
slideIndex&&(options.interval=!1),$target.carousel(options),(slideIndex=$this.attr("data-bs-slide-to"))&&$target.data("bs.carousel").to(slideIndex),e.preventDefault()}),$(window).on("load",function(){$('[data-bs-ride="carousel"]').each(function(){var $carousel=$(this);$carousel.carousel($carousel.data())})})}(jQuery),+function($){"use strict";var Collapse=function(element,options){this.$element=$(element),this.options=$.extend({},Collapse.DEFAULTS,options),this.transitioning=null,this.options.parent&&(this.$parent=$(this.options.parent)),this.options.toggle&&this.toggle()
...
};return $.support.transition?(this.$element[dimension](0).one($.support.transition.end,$.proxy(complete,this)).emulateTransitionEnd(350),void 0):complete.call(this)}}},Collapse.prototype.toggle=function(){this[this.$element.hasClass("in")?"hide":"show"]()};var old=$.fn.collapse;$.fn.collapse=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.collapse"),options=$.extend({},Collapse.DEFAULTS,$this.data(),"object"==typeof option&&option);data||$this.data("bs.collapse",data=new Collapse(this,options)),"string"==typeof option&&data[option]()
})},$.fn.collapse.Constructor=Collapse,$.fn.collapse.noConflict=function(){return $.fn.collapse=old,this},$(document).on("click.bs.collapse.data-api","[data-bs-toggle=collapse]",function(e){var href,$this=$(this),target=$this.attr("data-target")||e.preventDefault()||(href=$this.attr("href"))&&href.replace(/.*(?=#[^\s]+$)/,""),$target=$(target),data=$target.data("bs.collapse"),option=data?"toggle":$this.data(),parent=$this.attr("data-parent"),$parent=parent&&$(parent);data&&data.transitioning||($parent&&$parent.find('[data-bs-toggle=collapse][data-parent="'+parent+'"]').not($this).addClass("collapsed"),$this[$target.hasClass("in")?"addClass":"removeClass"]("collapsed")),$target.collapse(option)
...
return $parent&&$parent.length?$parent:$this.parent()}var backdrop=".dropdown-backdrop",toggle="[data-bs-toggle=dropdown]",Dropdown=function(element){$(element).on("click.bs.dropdown",this.toggle)};Dropdown.prototype.toggle=function(e){var $this=$(this);if(!$this.is(".disabled, :disabled")){var $parent=getParent($this),isActive=$parent.hasClass("open");if(clearMenus(),!isActive){if("ontouchstart"in document.documentElement&&!$parent.closest(".navbar-nav").length&&$('<div class="dropdown-backdrop"/>').insertAfter($(this)).on("click",clearMenus),$parent.triggerHandler(e=$.Event("show.bs.dropdown")),e.isDefaultPrevented())return;
...
},Modal.prototype.show=function(_relatedTarget){var that=this,e=$.Event("show.bs.modal",{relatedTarget:_relatedTarget});this.$element.triggerHandler(e),this.isShown||e.isDefaultPrevented()||(this.isShown=!0,this.escape(),this.$element.on("click.dismiss.modal",'[data-bs-dismiss="modal"]',$.proxy(this.hide,this)),this.backdrop(function(){var transition=$.support.transition&&that.$element.hasClass("fade");that.$element.parent().length||that.$element.appendTo(document.body),that.$element.show(),transition&&that.$element[0].offsetWidth,that.$element.addClass("in").attr("aria-hidden",!1),that.enforceFocus();
...
data||$this.data("bs.modal",data=new Modal(this,options)),"string"==typeof option?data[option](_relatedTarget):options.show&&data.show(_relatedTarget)})},$.fn.modal.Constructor=Modal,$.fn.modal.noConflict=function(){return $.fn.modal=old,this},$(document).on("click.bs.modal.data-api",'[data-bs-toggle="modal"]',function(e){var $this=$(this),href=$this.attr("href"),$target=$($this.attr("data-target")||href&&href.replace(/.*(?=#[^\s]+$)/,"")),option=$target.data("modal")?"toggle":$.extend({remote:!/#/.test(href)&&href},$target.data(),$this.data());
...
},self.options.delay.hide),void 0):self.hide()},Tooltip.prototype.show=function(){var e=$.Event("show.bs."+this.type);if(this.hasContent()&&this.enabled){if(this.$element.triggerHandler(e),e.isDefaultPrevented())return;var $tip=this.tip();this.setContent(),this.options.animation&&$tip.addClass("fade");var placement="function"==typeof this.options.placement?this.options.placement.call(this,$tip[0],this.$element[0]):this.options.placement,autoToken=/\s?auto?\s?/i,autoPlace=autoToken.test(placement);autoPlace&&(placement=placement.replace(autoToken,"")||"top"),$tip.detach().css({top:0,left:0,display:"block"}).addClass(placement.split("-")[0]),this.options.container?$tip.appendTo(this.options.container):$tip.insertAfter(this.$element);
...
data||$this.data("bs.scrollspy",data=new ScrollSpy(this,options)),"string"==typeof option&&data[option]()})},$.fn.scrollspy.Constructor=ScrollSpy,$.fn.scrollspy.noConflict=function(){return $.fn.scrollspy=old,this},$(window).on("load",function(){$('[data-bs-spy="scroll"]').each(function(){var $spy=$(this);$spy.scrollspy($spy.data())})})}(jQuery),+function($){"use strict";var Tab=function(element){this.element=$(element)};Tab.prototype.show=function(){var $this=this.element,$ul=$this.closest("ul:not(.dropdown-menu)"),selector=$this.data("target");
...
},$.fn.htmlData=function(prefix,nested){var parts,nested=void 0===nested?!0:nested,re=new RegExp("^data-"+(prefix?prefix+"-":"")+"(.*)","i"),data={};return $.each(this[0].attributes,function(i,attr){if(attr.specified&&(parts=attr.name.match(re))&&parts[1])if(nested){var i,prop,props=parts[1].split("-"),obj=data;for(max=props.length-1,i=0;max>=i;i++)prop=props[i],i==max?obj[prop]=attr.value:(!obj[prop]&&(obj[prop]={}),obj=obj[prop])}else data[parts[1]]=attr.value}),data},$.fn.defineData=function(name,value){return void 0===this.data(name)&&this.data(name,value),this
},$.deletes=function(obj,props){$.each(props,function(i,prop){delete obj[prop]})},$.fn.disabled=function(state){return void 0===state?this.is(":disabled")||this.hasClass("disabled"):this.prop("disabled",!!state).toggleClass("disabled",!!state)},$.fn.enabled=function(state){return void 0===state?!this.disabled():this.disabled(!state)},$.distinct=function(items){var uniqueElements=$.unique;if(!(items.length<1)){if(items[0].nodeType)return uniqueElements.apply(this,arguments);if("object"==typeof items[0]){var unique=Math.random(),uniqueObjects=[];
return $.each(items,function(i){items[i][unique]||(uniqueObjects.push(items[i]),items[i][unique]=!0)}),$.each(uniqueObjects,function(i){delete uniqueObjects[i][unique]}),uniqueObjects}return $.grep(items,function(item,i){return $.inArray(item,items)===i})}},$.fn.tagName=function(){return(this[0]||{}).tagName},$.create=function(tagName){return $(document.createElement(tagName))},$.fn.editable=function(editable){return $.isUndefined(editable)?"true"===this.prop("contenteditable"):(this.prop("contenteditable",editable),editable===!1&&this.removeAttr("contenteditable"),this)
},$.download=function(src){return $("<iframe>").hide().appendTo("body").bind("load",function(){$(this).remove()}).attr("src",src)},$.ns=function(event,ns){return event.split(" ").join(ns+" ")+ns},function(){var instance="___eventable",publicMethods=["on","off","fire"],getEventName=function(name){return name.split(".")[0]},Eventable=function(mode){this.fnList={},this.events={},this.mode=mode};$.extend(Eventable.prototype,{createEvent:function(name){return this.events[name]=$.Callbacks(this.mode)},on:function(name,fn){if(!name||!$.isFunction(fn))return this;
var fnList=this.fnList;(fnList[name]||(fnList[name]=[])).push(fn);var basename=getEventName(name);return(this.events[basename]||this.createEvent(basename)).add(fn),this},off:function(name){if(!name)return this;var basename=getEventName(name),event=this.events[basename];if(!event)return this;var removeCallbacks=function(fnList){$.each(fnList,function(i,fn){event.remove(fn)})};return basename!==name?$.each(this.fnList,function(name,fnList){name.indexOf(basename)>-1&&removeCallbacks(fnList)}):removeCallbacks(this.fnList[name]),this
...
}}(),$.fn.checkList=function(options){var defaultOptions={checkbox:".checkbox",masterCheckbox:".master-checkbox",check:function(){},uncheck:function(){},change:function(){}},options=$.extend({},defaultOptions,options),checkList=this,checkboxes=checkList.find(options.checkbox),masterCheckbox=checkList.find(options.masterCheckbox),disableChangeEvent=!1,change=function(){if(!disableChangeEvent){var checked=checkboxes.filter(":checked"),unchecked=checkboxes.not(":checked");checked.length<1&&masterCheckbox.removeAttr("checked"),checked.length==checkboxes.length&&masterCheckbox.prop("checked",!0),options.change.call(checkList,checked,unchecked)
}};return checkboxes.checked(function(){options.check.apply(checkList),change()},function(){options.uncheck.apply(checkList),change()}),masterCheckbox.checked(function(){disableChangeEvent=!0,checkboxes.checked(!0),disableChangeEvent=!1,change()},function(){disableChangeEvent=!0,checkboxes.checked(!1),disableChangeEvent=!1,change()}),change(),this},$.fn.checked=function(checked,unchecked){return arguments.length<1?this.is(":checked"):(this.each(function(){var input=$(this);return"boolean"==typeof checked?(input.attr("checked",checked).trigger("change"),void 0):((input.is("input[type=checkbox]")||input.is("input[type=radio]"))&&input.off("change.checked").on("change.checked",function(){try{return input.is(":checked")?checked.apply(input):unchecked.apply(input)
}catch(e){}}),void 0)}),this)},$.fn.locate=function(key){var prefix="data";return $.each(this[0].attributes,function(i,attr){return attr.specified&&"$"===attr.value?(prefix=attr.name,!1):void 0}),this.find("["+prefix+"-"+key.replace(/([a-z])([A-Z])/g,"$1-$2").toLowerCase()+"]")},function(){var props=["overflow","overflow-x","overflow-y"];$.fn.noscroll=function(lock){return void 0===lock&&(lock=!0),this.each(function(){var el=$(this),overflow=el.data("noscroll");!overflow&&lock&&(overflow={},$.each(props,function(i,prop){overflow[prop]=el.css(prop)
}),el.data("noscroll",overflow)),lock?$.each(props,function(i,prop){el.css(prop,"hidden")}):overflow&&el.css(overflow)})}}(),$.fn.visible=function(partial){var $t=$(this),$w=$(window);if(!($t.length<1)){var viewTop=$w.scrollTop(),viewBottom=viewTop+$w.height(),_top=$t.offset().top,_bottom=_top+$t.height(),compareTop=partial===!0?_bottom:_top,compareBottom=partial===!0?_top:_bottom;return viewBottom>=compareBottom&&compareTop>=viewTop}},$.fn.where=$.fn.filterBy=function(key,val,operator){var operator=operator||"=",selector="[data-"+key.replace(/([a-z])([A-Z])/g,"$1-$2").toLowerCase()+operator+val+"]";
return this.filter(selector)},$.fn.checked=function(checked,unchecked){return arguments.length<1?this.is(":checked"):(this.each(function(){var input=$(this);return"boolean"==typeof checked?(input.attr("checked",checked).trigger("change"),void 0):((input.is("input[type=checkbox]")||input.is("input[type=radio]"))&&input.off("change.checked").on("change.checked",function(){try{return input.is(":checked")?checked.apply(input):unchecked.apply(input)}catch(e){}}),void 0)}),this)},$.fn.selectAll=function(){return this.each(function(){this.select()
})},$.fn.unselect=function(){return this.each(function(){var input=this,value=input.value;input.value+=" ",input.value=value})},$.fn.toObject=$.fn.serializeObject=function(){var obj={};return $.each($(this).serializeArray(),function(i,prop){obj.hasOwnProperty(prop.name)?($.isArray(obj[prop.name])||(obj[prop.name]=[obj[prop.name]]),obj[prop.name].push(prop.value)):obj[prop.name]=prop.value}),obj},$.fn.toJSON=$.fn.serializeJSON=function(){return JSON.stringify($(this).serializeObject())},$.sanitizeHTML=function(html){return $($.parseHTML(html,document,!0)).toHTML()
},$.toHTML=function(str){return $("<div>").html(str).html()},$.fn.toHTML=function(){return $.toHTML(this.clone())},$.toXHTML=function(obj,maintainUppercaseTag){var zz=obj.innerHTML?String(obj.innerHTML):obj,z=zz.match(/(<.+[^>])/g);if(z)for(var i=0;i<z.length;i+=1){var y,zSaved=z[i],attrRE=/\=[a-zA-Z\.\:\[\]_\(\)\&\$\%#\@\!0-9\/]+[?\s+|?>]/g;if(z[i]=z[i].replace(/([<|<\/].+?\w+).+[^>]/,function(a){return a}),y=z[i].match(attrRE))for(var j=0,len=y.length;len>j;){var replaceRE=/(\=)([a-zA-Z\.\:\[\]_\(\)\&\$\%#\@\!0-9\/]+)?([\s+|?>])/g,replacer=function(){var args=Array.prototype.slice.call(arguments);
return'="'+(maintainUppercaseTag?args[2]:args[2].toLowerCase())+'"'+args[3]};z[i]=z[i].replace(y[j],y[j].replace(replaceRE,replacer)),j+=1}zz=zz.replace(zSaved,z[i])}return zz},$.fn.xhtml=function(){return $.IE?$.toXHTML(this[0]):this.html()},$.buildHTML=function(html,keepScripts){if(html instanceof $)return html;var doc=document;if(window.CloudFlare){var iframe=$.buildHTML.iframe;if(!iframe||!iframe.contentDocument){var iframe=$.buildHTML.iframe=document.createElement("iframe");iframe.style.display="none",document.body.appendChild(iframe)
...
})}try{var testKey="__storejs__";store.set(testKey,testKey),store.get(testKey)!=testKey&&(store.disabled=!0),store.remove(testKey)}catch(e){store.disabled=!0}store.enabled=!store.disabled,$.Storage=store}),FD40.plugin("uri",function($){$.isUrl=function(s){var regexp=/^(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;return regexp.test(s)};var Query=function(queryString){"use strict";var parseQuery=function(q){var i,ps,p,kvp,k,v,arr=[];if("undefined"==typeof q||null===q||""===q)return arr;
