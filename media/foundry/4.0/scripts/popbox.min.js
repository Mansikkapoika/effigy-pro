...
})}var win=$(window),repositionOnResize="resize.popbox"+popbox.uid;if(win.off(repositionOnResize).on(repositionOnResize,function(){popbox.tooltip.length>0&&popbox.tooltip.position(popbox.position)}),popbox.tooltip.length>0)return popbox.tooltip.appendTo("body").position(popbox.position),popbox.trigger("popboxActivate",[popbox]),void 0;if($.isFunction(popbox.content)){var options=popbox.content(popbox);if(popbox.update(options),popbox.wait)return}$.isDeferred(popbox.content)&&("pending"==popbox.content.state()&&(popbox.loader.appendTo("body").position(popbox.position),popbox.trigger("popboxLoading",[popbox])),popbox.content.always(function(){popbox.wait=!1
}).done(function(html){if(!(popbox.tooltip.length>0)&&popbox.enabled){popbox.loader.detach();var tooltip=$.buildHTML(html);if(tooltip.filter("[data-popbox-tooltip]").length<1){var content=tooltip;tooltip=$('<div id="fd" class="popbox" data-popbox-tooltip><div class="arrow"></div><div class="popbox-content" data-popbox-content></div></div>').attr({id:popbox.id,"data-popbox-tooltip":popbox.type,style:popbox.fixed?"position: fixed":""}).addClass(popbox.component).addClass("popbox-"+popbox.type).appendTo("body"),tooltip.find("[data-popbox-content]").append(content)
