void function ( exports, $, _, Backbone ) {
	...
	exports.ES_Standard_Controller = ES_Slider_Controller.extend({
		initialize: function ( options ) {
			...
			function render_Switch( IN, OUT, axis, size, progress, vector ) {
				...
				var rotationVector = axis == 'y' ? '0,1,0' : '1,0,0';
				var radius = size / 2;
				transform(OUT, 'translate3d(0,0,-' + radius + 'px) ' +
					'rotate3d(' + rotationVector + ',' + (outRotation) + 'deg) ' +
					'translate3d(0,0,' + radius + 'px) ' +
					'rotate3d(' + rotationVector + ',' + (-outRotation) + 'deg)');
				transform(IN, 'translate3d(0,0,-' + radius + 'px) ' +
					'rotate3d(' + rotationVector + ',' + (inRotation) + 'deg) ' +
					'translate3d(0,0,' + radius + 'px) ' +
					'rotate3d(' + rotationVector + ',' + (-inRotation) + 'deg)');
			};
		...
		initialize: function ( options ) {
			...
			switch ( mode ) {
				case 'cube':
					...
					break;
				case 'carousel':
					slider.$el.addClass('es-flat-slider')
	...
	exports.ES_Cube_Controller = function ( slider ) {
        ...
        var flow = slider.model.get('layout.flow');
		var slides = slider.slides.subViews;
		var $cube = $('<div class="es-cube">').appendTo(slider.$stage)

		slider.$el.addClass('es-cube-slider');
		slider.$('.es-viewport').css('overflow', 'visible');
		...
		slider.on('slide:transition', function ( slide, direction, side, progress ) {
			...
			switch ( direction ) {
				...
				case 'out': // The active slide facing front
                    ...
                    if ( flow == 'x' || flow == 'X') {
                        slide.$el.css('transform', 'translate3d(0,0,' + (size / 2) + 'px)');
                        $cube.css('transform', 'translate3d(0,0,' + (-size / 2) + 'px) rotate3d(0,1,0,' + rotation + 'deg)');
                    }
                    else if (flow == 'y' || flow == 'Y') {
                        slide.$el.css('transform', 'translate3d(0,0,' + (size / 2) + 'px)');
                        $cube.css('transform', 'translate3d(0,0,' + (-size / 2) + 'px) rotate3d(1,0,0,' + -rotation + 'deg)');
                    }
				...
				case 'in': // The next slide facing left or right
                    ...
                    switch (flow) {
                        case 'x':
                        case 'X':
                            slide.$el.css('transform', 'translate3d(' + faceOffsetX + 'px,0,0) rotate3d(0,1,0,' + faceRotation + 'deg)');
                            break
                        case 'y':
                        case 'Y':
                            slide.$el.css('transform', 'translate3d(0,' + faceOffsetX + 'px,0) rotate3d(1,0,0,' + -faceRotation + 'deg)');
                            break
                    }
	...
	exports.ES_Carousel_Controller = function ( slider ) {
		...
		var radius;
		var slides = slider.slides.subViews;
		var $carousel = $('<div class="es-carousel">').appendTo(slider.$stage);
        var flow = slider.model.get('layout.flow');

		slider.$el
			.addClass('es-carousel-slider')
		...
		slider.on('resize', function () {
			...
			slider.setPerspective(slider.stage_size)
		});
		slider.on('progress', function ( progress ) {
            ...
            switch ( flow.toLowerCase() ) {
                case 'x':
                    $carousel.css({
                        transform: 'translate3d(0,0,-' + radius + 'px) rotate3d(0,1,0,' + rotation + 'deg)'
                    });
                    _.each(slides, function ( slide ) {
                        slide.$el.css({
                ...
                case 'y':
                    $carousel.css({
                        transform: 'translate3d(0,0,-' + radius + 'px) rotate3d(1,0,0,' + -rotation + 'deg)'
                    });
                    _.each(slides, function ( slide ) {
                        slide.$el.css({
	...
	exports.ES_Polygon_Controller = function ( slider ) {
		...
		slider.$global.hide();
		slider.$el.addClass('es-carousel-slider');
		_.each(slides, function ( slide, index ) {
			slide.angle = index / slides.length * 360;
			slide.$el.appendTo($carousel);
			slide.load()
	...
	exports.ES_Coverflow_Controller = function ( slider ) {
		...
		slider.on('resize', function () {
			...
			slider.$stage.css('transform', 'translate3d(0,0,-' + depth + 'px)')
			slider.getActiveSlide().$el.css('transform', 'translate3d(0,0,' + depth + 'px)')
		...
		slider.on('slide:transition', function ( slide, direction, side, progress ) {
			if ( slider.activeIndex == slide.index ) {
				_.each(slides, function ( otherSlide ) {
					if ( otherSlide.offsetIndex > slide.offsetIndex + 1 )
						otherSlide.$el.css('transform', 'rotate3d(0,1,0,-' + angle + 'deg)');
					if ( otherSlide.offsetIndex < slide.offsetIndex - 1 )
						otherSlide.$el.css('transform', 'rotate3d(0,1,0,' + angle + 'deg)');
				})
			}
			...
			switch ( side ) {
				case 'left':
					slide.$el.css('transform', 'translate3d(0,0,' + position + 'px) rotate3d(0,1,0,' + (rotation) + 'deg)');
					break;
				case 'right':
					slide.$el.css('transform', 'translate3d(0,0,' + position + 'px) rotate3d(0,1,0,' + (-rotation) + 'deg)');
					break;
